...
let current_path = Process().currentDirectoryPath
...
struct BERTVocabulary {
    ...
    private static func loadVocabulary() -> [Substring: Int] {
        //let fileName = current_path + "/Resources/" + "bert-base-uncased-vocab"
        let fileName = "Sources/BertNlpDbpediaInterface_swift/Resources/bert-base-uncased-vocab.txt"
        let expectedVocabularySize = 30_522
         guard let rawVocabulary = try? String(contentsOfFile: fileName) else {
